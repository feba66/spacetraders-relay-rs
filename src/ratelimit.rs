/*
Original from https://docs.rs/crate/ratelimit/latest
with the MIT license http://opensource.org/licenses/MIT

modified to use a sliding window approach
*/
...
impl Ratelimiter {
    ...
    fn refill(&self, time: Instant) -> Result<(), core::time::Duration> {
        ...
        loop {
            ...
            let next_refill = time
                + clocksource::Duration::<Nanoseconds<u64>>::from_nanos(
                    parameters.refill_interval.as_nanos(),
                );
